...
BIONANO = ["Bionano 光学图谱简介","Bionano 光学图谱是利用酶切和荧光标记对长达几百Kb 的单链DNA 分子进行成像，组装得到物理图谱。其主要流程为：（1）用内切酶识别特定的位点，单链切割，荧光标记后用聚合酶修复单链缺口；（2）荧光标记的DNA 分子在电场下的纳米通道内被拉直荧光成像；（3）单分子荧光成像，获得酶切位点分布图谱CMAP；（4）通过Irys分析软件组装得到全基因组的连锁图谱。"]
